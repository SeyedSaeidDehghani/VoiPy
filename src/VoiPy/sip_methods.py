...
class SipRegister(Method):
    ...
    def __refresh_register(self) -> None:
        ...
        self.cseq_id = self.register_counter.next()
        request = self.sip.request_creator.gen_register(branch=self.branch, call_id=self.call_id, tag=self.tag,
                                                        response=self.auth_info["response"],
                                                        nonce=self.auth_info["nonce"], cseq_id=self.cseq_id)
...
class SipInvite(Method):
    def __init__(self, parent):
        ...
        self.invite_counter = self.sip.invite_counter
        self.call_id: str = ''
        self.auth_info: dict = {}
        self.branch: str = ''
        self.tag: str = ''
    ...
    def run(
        ...
        while self._state < 3:
            i += 1
            # if self._state == 3:
            #     return sip_message.SipParseMessage(helper.list_to_string(self.request)), self.call_id, self.session_id, self.auth_info
            if i > 20:
                if self._state == 3:
...
class SipTransfer(Method):
    def __init__(self, parent):
        ...
        self.medias: dict = {}
        self.send_type: rtp.TransmitType = rtp.TransmitType.SENDRECV
        self.refer_counter = self.sip.invite_counter
        self.call_id: str = ''
        self.auth_info: dict = {}
        self.branch: str = ''
        self.tag_from: str = ''
    ...
    def run(
        ...
        return sip_message.SipParseMessage(result)
    ...
    def __refer(self) -> None:
        ...
        response_hash = None
        if self.auth_info:
            response_hash = self.sip.create_hash(nonce=self.auth_info["nonce"],
                                                 method="REFER", call_to=self.refer_to)
            nonce = helper.quote(self.auth_info["nonce"])
        self.branch = "z9hG4bK" + self.sip.gen_callId()[0:25]
        self.cseq_id = self.refer_counter.next()

        request = self.sip.request_creator.gen_refer(number=self.number, cseq_id=self.cseq_id,
    ...
    def __bye(self):
        ...
        self.sip.bye(request, cseq_id=self.cseq_id + 2)
